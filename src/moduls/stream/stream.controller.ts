...
router.get(
    'stream/:magnet/:filename', (req: StreamRequest, res: Response, next: NextFunction) => {
        ...
        if(!range) {
            const err = new Error('Range is not defined, please make request from HTML5') as ErrorWithStatus
            err.status = 416
            return next(err)
        }
